...
void Statistics::init_vars()
{
  ...
  for (int i=0; i<MAX_PLAYERS_NAME; i++) {
    first_place[i] = 0;
    second_place[i] = 0;
    third_place[i] = 0;
    fourth_place[i] = 0;
...
int Statistics::save_stats_file()
{
  QFile file(QDir::homePath() + STATS_FILENAME);
  ...
  out << game_started << " " << game_finished << " " << hands_played << " " << undo << "\n";
  ...
  for (int i=0; i<MAX_PLAYERS_NAME; i++) {
     out << first_place[i]           << " " << second_place[i]        << " " <<
            ...
            no_tricks_bonus_taken[i] << " " << perfect_100_bonus_taken[i] << "\n";
  }
...
int Statistics::load_stats_file()
{
  QFile file(QDir::homePath() + STATS_FILENAME);
...
void Statistics::update_window(int plr, int stats)
{
...
void Statistics::increase_stats(int plr, int stats)
{
  switch (stats) {
...
void Statistics::set_score(int plr, int score)
{
...
void Statistics::initTableContent()
{
...
bool Statistics::is_file_corrupted()
{
  return file_corrupted;
}

void Statistics::Translate()
{
